...
-----------------------------------------------------------------------------------------
function XmlParser:GetNode(Object,node)
 for i,xmlNode in pairs(Object.ChildNodes) do
	if(xmlNode.Name==node) then
		 return xmlNode
	end
	--debug_in_file(xmlNode.Name .. '\n');
 end
 return nil
end
-----------------------------------------------------------------------------------------
function XmlParser:dump(_class, no_func, depth)
	if(not _class) then 
		debug_in_file("nil" .. '\n');
		return;
	end
	...
	debug_in_file(str.."["..type(_class).."]" .. '\n');
	debug_in_file(str.."{" .. '\n');
    
	for i,field in pairs(_class) do
		if(type(field)=="table") then
			debug_in_file(str.."\t"..tostring(i).." =" .. '\n');
			XmlParser:dump(field, no_func, depth+1);
		else 
			if(type(field)=="number") then
				debug_in_file(str.."\t"..tostring(i).."="..field .. '\n');
			elseif(type(field) == "string") then
				debug_in_file(str.."\t"..tostring(i).."=".."\""..field.."\"" .. '\n');
			elseif(type(field) == "boolean") then
				debug_in_file(str.."\t"..tostring(i).."=".."\""..tostring(field).."\"" .. '\n');
			else
				if(not no_func)then
					if(type(field)=="function")then
						debug_in_file(str.."\t"..tostring(i).."()" .. '\n');
					else
						debug_in_file(str.."\t"..tostring(i).."<userdata=["..type(field).."]>" .. '\n');
					end
				end
			end
		end
