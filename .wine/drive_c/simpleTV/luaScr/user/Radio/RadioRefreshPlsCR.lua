--script for https://www.classicalradio.com/ (03/10/2019)
...
-------------------------------------------
 m_simpleTV.OSD.ShowMessage("Classicalradio - start updating playlist" ,0xFF00,10)

local weblink_view = ''

if source == 2 then

 local rc,answer=m_simpleTV.WinInet.Request(session,{url='https://cloud.mail.ru/public/4KYo/55jc6Vrvn/RadioAddonLogo/Classicalradio/'})
...
weblink_view = string.match(answer,'weblink_view.-url":.-"(.-)"') or ''
 ...
 local rc,answer=m_simpleTV.WinInet.Request(session,{url='https://www.classicalradio.com/'})
 --m_simpleTV.WinInet.Close(session)
 
 if rc~=200 then
 	   m_simpleTV.WinInet.Close(session)
 	   m_simpleTV.OSD.ShowMessage("Classicalradio ñonnection error - " .. rc ,255,10)
 	   return
  end
 --debug_in_file(answer .. '\n')
 ...
 if answer == nil then 
    m_simpleTV.OSD.ShowMessage("Error Classicalradio - json not found, abort"  ,255,5)
    return
 end
 ...
 local i=1
 local m3ustr = '#EXTM3U $LogoPath="..\\Channel\\logo\\RadioAddonLogo\\Classicalradio\\"\n'
 local name, adr, desc
 local grp = 'Classicalradio Web'
 local img=''
 local UpdateID='Classicalradio'

 while true do
   if t.channels[i] == nil then break end
     name =  t.channels[i].name
     adr  = 'classicalradioid=' .. t.channels[i].id  
...
--[[
 local url = 'https://api.friezy.ru/playlists/m3u/CR.m3u'
 ...
 if rc~=200 then
	   m_simpleTV.WinInet.Close(session)
	   m_simpleTV.OSD.ShowMessage("Classicalradio ñonnection error - " .. rc ,255,3)
	   return
 end
 --debug_in_file(answer .. '\n')

 if not findpattern(answer, '#EXTM3U',1,0,0) then
	   m_simpleTV.OSD.ShowMessage("Classicalradio - unable to load premium playlist" ,255,3)
    answer=''
 end

if findpattern(answer, '#EXTM3U',1,0,0) then
   answer = findpattern(answer, '^(.+)',129,0,0) or ''
   answer = string.gsub (answer, '#EXTINF.-,','#EXTINF:-1 skipepg="1" group-title="Classicalradio 320kbps",')
end
