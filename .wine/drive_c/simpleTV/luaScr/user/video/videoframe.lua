-- видеоскрипт для видеобазы "videoframe" (25/10/19)
-- открывает подобные ссылки:
-- https://videoframe.at/frameindex.php?kp=5928
-- https://videoframe.at/movie/4204258p119/iframe
	...
	if m_simpleTV.User.videoframe.Tabletitle then
		...
		if index then
			title = m_simpleTV.User.videoframe.title .. ' - ' .. m_simpleTV.User.videoframe.Tabletitle[index+1].Name
		end
	end
	local host = inAdr:match('(https?://.-)/')
	local refer = 'https://smotrihd.com/'
	...
	local function GetvideoframeAddress(answer)
		...
		local retAdr = host .. '/loadvideo'
		local xref = refer:match('https?://(.-)/')
		local headers = 'X-REF: ' .. xref .. '\nContent-Type:application/x-www-form-urlencoded;charset=UTF-8\nReferer: ' .. refer
		local rc, answer = m_simpleTV.Http.Request(session, {body = body, url = retAdr, method = 'post', headers = headers})
			if rc ~= 200 or (rc == 200 and answer == '') then return end
		local validate = answer:match('"validate":"(.-)"')
		if validate then
			m_simpleTV.OSD.ShowMessageT({text = 'ожидайте 15 сек.', color = ARGB(255, 255, 127, 80), showTime = 1000 * 15, id = 'channelName'})
		...
		local qlty, adr
			for w in answer:gmatch('#EXT%-X%-STREAM%-INF:(.-m3u8)') do
				...
				t[i] = {}
				adr = adr:gsub('%.mp4.+', '.mp4')
				t[i].Adress = adr:gsub('manifest%.', 'index.'):gsub('https://', 'http://') .. '$OPT:NO-STIMESHIFT'
				t[i].qlty = qlty
				i = i + 1
			end
...
-- debug_in_file(retAdr .. '\n')
	...
	local rc, answer = m_simpleTV.Http.Request(session, {url = inAdr:gsub('%$videoframe', ''), headers = 'Referer: ' .. refer})
		if rc == 404 and answer and answer:match('Sorry') then
			m_simpleTV.Http.Close(session)
			m_simpleTV.OSD.ShowMessageT({text = 'Контент недоступен в вашем регионе', color = ARGB(255, 155, 255, 155), showTime = 1000 * 10, id = 'channelName'})
		...
		if rc ~= 200 then
			m_simpleTV.Http.Close(session)
			m_simpleTV.OSD.ShowMessageT({text = 'videoframe ошибка[1]-' .. rc, color = ARGB(255, 155, 255, 155), showTime = 1000 * 5, id = 'channelName'})
		 return
		end
		if inAdr:match('%$videoframe') then
			play(answer, title)
	...
	local typ = answer:match('type = \'(.-)\'')
		if not typ then
			m_simpleTV.Http.Close(session)
			m_simpleTV.OSD.ShowMessageT({text = 'videoframe ошибка[2]', color = ARGB(255, 155, 255, 155), showTime = 1000 * 5, id = 'channelName'})
		 return
		end
	...
	if transl then
		...
		if i > 2 then
			...
			rc, answer = m_simpleTV.Http.Request(session, {url = inAdr, headers = 'Referer: ' .. refer})
				if rc ~= 200 then
					m_simpleTV.Http.Close(session)
					m_simpleTV.OSD.ShowMessageT({text = 'videoframe ошибка[3]-' .. rc, color = ARGB(255, 155, 255, 155), showTime = 1000 * 5, id = 'channelName'})
				 return
				end
		end
	end
	if typ == 'serial' then
		...
		if sesons then
			local t, i, nameepi = {}, 1
				for adress, nameseasons in sesons:gmatch('<a href=\'(.-)\'.->(.-)</a>') do
					t[i] = {}
					t[i].Name = nameseasons
					t[i].Adress = host .. adress
					i = i + 1
				end
			t = table_reverse(t)
			for i = 1, #t do
			...
			rc, answer = m_simpleTV.Http.Request(session, {url = inAdr})
				if rc ~= 200 then
					m_simpleTV.Http.Close(session)
					m_simpleTV.OSD.ShowMessage_UTF8('videoframe ошибка[3] - ' .. rc, 255, 5)
				 return
				end
		end
		...
		if serias then
			local t, i = {}, 1
				for adress, nameserias in serias:gmatch('<a href=\'(.-)\'.->(.-)</a>') do
					t[i] = {}
					t[i].Name = nameserias
					t[i].Adress = '$videoframe' .. host .. adress
					i = i + 1
				end
			t = table_reverse(t)
			for i = 1, #t do
			...
			rc, answer = m_simpleTV.Http.Request(session, {url = inAdr:gsub('/$iframe', '/iframe'):gsub('%$videoframe', '')})
				if rc ~= 200 then
					m_simpleTV.Http.Close(session)
					m_simpleTV.OSD.ShowMessageT({text = 'videoframe ошибка[4]-' .. rc, color = ARGB(255, 155, 255, 155), showTime = 1000 * 5, id = 'channelName'})
				 return
				end
		end
	else
		...
		m_simpleTV.OSD.ShowSelect_UTF8('Videoframe', 0, t1, 5000, 64+32+128)
	end
	play(answer, title)