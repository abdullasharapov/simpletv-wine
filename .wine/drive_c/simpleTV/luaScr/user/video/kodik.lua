-- видеоскрипт для видеобазы "kodik" http://kodik.cc (3/11/19)
...
-- http://kodik.info/serial/13166/dc6c81648d5b5173461756cf1f2cd0e4/720p
	...
	if m_simpleTV.User.kodik.Tabletitle then
		local index = m_simpleTV.Control.GetMultiAddressIndex()
		if index then
			title = m_simpleTV.User.kodik.title .. ' - ' .. m_simpleTV.User.kodik.Tabletitle[index+1].Name
		end
	end
...
-- debug_in_file(retAdr .. '\n')
	...
	local rc, answer = m_simpleTV.Http.Request(session, {url = inAdr, headers = 'Referer: ' .. refer})
		if rc ~= 200 then
			m_simpleTV.Http.Close(session)
			m_simpleTV.OSD.ShowMessageT({text = 'kodik ошибка[1]-' .. rc, color = ARGB(255, 155, 255, 155), showTime = 1000 * 5, id = 'channelName'})
		 return
		end
	local season_title = ''
	local seson = ''
	...
	if episodes then
		...
		inAdr = answer:match('<iframe src="(.-)"') or answer:match('iframe.src = "(.-)"')
			if not inAdr then
				m_simpleTV.Http.Close(session)
				m_simpleTV.OSD.ShowMessageT({text = 'kodik ошибка[3]', color = ARGB(255, 155, 255, 155), showTime = 1000 * 5, id = 'channelName'})
			 return
			end
		local t1 = {}
		t1[1] = {}
