-- видеоскрипт для сайта http://www.lostfilm.tv (12/11/19)
-- необходим: Acestream
...
-- http://www.lostfilm.tv/series/The_Punisher/video/2
	...
	local inAdr = m_simpleTV.Control.CurrentAdress
		...
		if not inAdr:match('^https?://www.lostfilm%.tv/series/')
	...
	if m_simpleTV.User.lostfilm.Tabletitle then
		...
		if index then
			title = m_simpleTV.User.lostfilm.title .. ' - '
			.. m_simpleTV.User.lostfilm.Tabletitle[index + 1].Name
		end
	end
	...
	local session = m_simpleTV.Http.New('Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3729.108 Safari/537.36')
