-- видеоскрипт для сайта http://www.ivi.ru (28/10/19)
...
------------------------------------------------------------------------------------------
	...
	local function GetiviAddress(videoid)
		...
		local i, b, t, adr, name, rtd = 1, 1, {}
			while true do
				...
				if not name:match('DASH') then
					...
					t[i].Adress = adr .. '$OPT:NO-STIMESHIFT' .. captions
					t[i].Name = name
					t[i].res = rtd
					i = i + 1
				end
		...
		elseif index < m_simpleTV.User.ivi.Index then
			...
			retAdr = t[m_simpleTV.User.ivi.Index].Adress
		end
		if not retAdr then
			retAdr = t[1].Adress
		end
		...
		if not answer:match('"error"') then
			...
			if tab.result.seasons[1] and not inAdr:match('/season(%d+)') then
				local t, i = {}, 1
					while true do
						...
						t[i].Adress = tab.result.seasons[i]
						i = i + 1
					end
				if i > 2 then
			...
			local t, i = {}, 1
				while true do
					...
					t[i].Adress = 'https://www.ivi.ru/id=' .. tab.result[i].id
					i = i + 1
				end
			...
			t1[1].Id = 1
			t1[1].Name = title
			t1[1].Adress = 'https://www.ivi.ru/id=' .. compilation
