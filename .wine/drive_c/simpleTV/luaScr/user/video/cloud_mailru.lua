-- видеоскрипт для https://cloud.mail.ru (1/07/19)
...
-- https://cloud.mail.ru/public/4VZcMg86gm1s/%D0%90%D1%83%D0%B4%D0%B8%D0%BE%D0%BA%D0%BD%D0%B8%D0%B3%D0%B8
	...
	local inAdr = m_simpleTV.Control.CurrentAdress
		...
		if not inAdr:match('^https://cloud%.mail%.ru/public/%w+')
			and not inAdr:match('&Mailru')
		then
		 return
		end
	...
	local title
		if inAdr:match('&Mailru') then
			if m_simpleTV.User.Mailru.Plst then
				...
				if index then
					title = m_simpleTV.User.Mailru.Plst[index + 1].Name
				end
			end
	...
	local retAdr, name, kind, link, count
		while true do
			...
			if kind == 'folder' then
				...
				tab[n].Adress = CloudMailCleanAddress('https://cloud.mail.ru/public/' .. link)
			...
			elseif kind == 'file' then
				...
				if is_find == true then
					...
					tab[n].Adress = CloudMailCleanAddress(host ..'/' .. link) .. '&Mailru'
					if not retAdr then
						IndexToplay = n - 1
						retAdr = tab[n].Adress
					end
		...
		if not retAdr then
			if #tab > 0 then
				...
				retAdr = tab[id].Adress .. '&folder=true'
