-- видеоскрипт для сайта https://media.1tv.ru (15/4/19)
-- открывает:
...
------------------------------------------------------------------------------------------
local bdw = 0 -- качество в кбит/с: 0 - максимальное; ограничить от 1000
------------------------------------------------------------------------------------------
		if m_simpleTV.Control.ChangeAdress ~= 'No' then return end
	local inAdr = m_simpleTV.Control.CurrentAdress
		if not inAdr then return end
		if not inAdr:match('^https?://www%.5%-tv%.ru/live')
			and not inAdr:match('^https?://www%.domashniy%.ru/online')
			and not inAdr:match('^https?://chetv%.ru/online')
			and not inAdr:match('^https?://ctclove%.ru/online')
			and not inAdr:match('^https?://ren%.tv/live')
			and not inAdr:match('^https?://ctc%.ru/online')
			and not inAdr:match('^https?://www%.1tv%.ru/live')
			and not inAdr:match('^https?://www%.1tv%.ru/embedlive')
			and not inAdr:match('^https?://stream.1tv%.ru/embed')
			and not inAdr:match('^https?://media%.1tv%.ru/embed/nmg/')
			and not inAdr:match('^https?://media%.1tv%.ru/embed/ctcmedia/')
			and not inAdr:match('^$tv1onln')
		then
		 return
		end
	require 'json'
	...
	if not m_simpleTV.User.tv1onln then
		m_simpleTV.User.tv1onln = {}
	end
	if not m_simpleTV.User.TVTimeShift then
		m_simpleTV.User.TVTimeShift = {}
	end
	if bdw > 0 and not m_simpleTV.User.tv1onln.MaxResolution then
		m_simpleTV.User.tv1onln.MaxResolution = bdw * 1000
	end
	if bdw == 0 and not m_simpleTV.User.tv1onln.MaxResolution then
		m_simpleTV.User.tv1onln.MaxResolution = 100000000
	end
	...
	if inAdr:match('^$tv1onln') then
		if m_simpleTV.User.tv1onln.ResolutionTable then
			local index = GetMaxResolutionIndex(m_simpleTV.User.tv1onln.ResolutionTable)
	...
	local array
	if inAdr:match('nmg%-iz') then
		array = 'nmg/playlist/iz'
	end
	if inAdr:match('ctc%-dom') or inAdr:match('domashniy%.ru/online') then
		array = 'ctc/playlist/ctc-dom'
	end
	if inAdr:match('nmg%-ren') or inAdr:match('ren%.tv/live') then
		array = 'nmg/playlist/ren-tv'
	end
	if inAdr:match('ctc%-ctc') or inAdr:match('ctc%.ru/online') then
		array = 'ctc/playlist/ctc'
	end
	if inAdr:match('ctc%-che') or inAdr:match('chetv%.ru/online') then
		array = 'ctc/playlist/ctc-che'
	end
	if inAdr:match('nmg%-5tv') or inAdr:match('5%-tv%.ru/live') then
		array = 'nmg/playlist/tv-5'
	end
	if inAdr:match('ctc%-love') or inAdr:match('ctclove%.ru/online') then
	array = 'ctc/playlist/ctc-love'
	end
	if inAdr:match('1tv%.ru/embedlive') or inAdr:match('www%.1tv%.ru/live') or inAdr:match('stream%.1tv%.ru/embed') then
		array = 'playlist/1tvch' api = api2
	end
	...
	if i > 2 then
		t[i] = {Id = i, Name = 'Авто', res = 100000000, Adress = '$tv1onln' .. retAdr}
